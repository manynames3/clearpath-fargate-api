...
The ALB remains useful as the regional origin because it integrates directly with ECS target groups, health checks, HTTPS listeners, and path-based routing.
...
- The ALB security group accepts HTTPS only from the CloudFront origin-facing managed prefix list.
